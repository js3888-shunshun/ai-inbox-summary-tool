...
describe("parseDigest", () => {
  it("parses a clean JSON object", () => {
    const d = parseDigest('{"headline":"3 asks need replies","body":"- Alice wants numbers"}', 3);
    expect(d).toEqual({ headline: "3 asks need replies", body: "- Alice wants numbers", messageCount: 3 });
  });

  it("tolerates ```json fences and surrounding prose", () => {
    const raw = 'Here you go:\n```json\n{"headline":"h","body":"b"}\n```\nThanks!';
    expect(parseDigest(raw, 1)).toMatchObject({ headline: "h", body: "b", messageCount: 1 });
  });
...
describe("ClaudeSummarizer", () => {
  ...
  it("assembles input, calls the model, and parses the output", async () => {
    const complete: CompletionFn = vi.fn(async (input) => {
      ...
      return '{"headline":"1 ask awaiting reply","body":"- Alice: send numbers by Friday"}';
    });
    const digest = await new ClaudeSummarizer(complete).summarize([msg()]);
    expect(digest).toEqual({
      headline: "1 ask awaiting reply",
      body: "- Alice: send numbers by Friday",
      messageCount: 1,
    });
  });
});
